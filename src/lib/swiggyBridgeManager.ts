...
export class SwiggyBridgeManager {
  ...
  constructor(private readonly config: BridgeConfig) {
    ...
    const metadata: OAuthClientMetadata = {
      client_name: this.config.swiggyClientName,
      redirect_uris: [callbackUrl],
      grant_types: ["authorization_code", "refresh_token"],
      response_types: ["code"],
      token_endpoint_auth_method: "none",
      scope: this.config.swiggyScope,
    };
  ...
  async beginLogin(): Promise<{ authorizationUrl?: string; alreadyAuthorized: boolean }> {
    this.pendingAuthorizationUrl = undefined;

    const bootstrapClient = new Client(
      {
        name: "swiggy-bridge-bootstrap",
        version: "0.1.0",
      },
      { capabilities: {} },
    );
    ...
    try {
      ...
      if (error instanceof UnauthorizedError) {
        if (!this.pendingAuthorizationUrl) {
          throw new Error(
            "Upstream Swiggy server requested OAuth but did not provide an authorization URL.",
          );
        }
  ...
  private registerControlTools(binding: LocalBinding): void {
    binding.server.registerTool(
      "bridge_status",
      {
        description: "Return bridge health, OAuth status, and the currently proxied Swiggy tools.",
        inputSchema: {},
      },
    ...
    binding.server.registerTool(
      "bridge_sync_tools",
      {
        description: "Refresh the proxied Swiggy tool catalog from upstream.",
        inputSchema: {
          reconnect: z
            .boolean()
            .optional()
    ...
    binding.server.registerTool(
      ...
      {
        description:
          "Return the admin login URL operators should open to complete or renew Swiggy OAuth for this bridge.",
        inputSchema: {},
      },
      async (): Promise<CallToolResult> => ({
        content: [
    ...
    try {
      ...
      const message =
        error instanceof UnauthorizedError
          ? `Swiggy OAuth is missing or expired. Visit ${this.getLoginUrl()} and retry.`
  ...
  private async ensureConnected(endpoint: EndpointConfig): Promise<UpstreamConnection> {
    ...
    const client = new Client(
      {
        name: `swiggy-bridge-upstream-${endpoint.key}`,
        version: "0.1.0",
      },
      { capabilities: {} },
    );
